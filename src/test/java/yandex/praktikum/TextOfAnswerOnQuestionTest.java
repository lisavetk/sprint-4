package yandex.praktikum;

import io.github.bonigarcia.wdm.WebDriverManager;
import org.junit.After;
import org.junit.Before;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.junit.runners.Parameterized;
import org.openqa.selenium.By;
import org.openqa.selenium.JavascriptExecutor;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.WebDriverWait;
import yandex.praktikum.pageObject.HomePageLocators;

import java.time.Duration;

import static org.junit.Assert.assertEquals;

@RunWith(Parameterized.class)
public class TextOfAnswerOnQuestionTest {
    private WebDriver driver;
    private final String expectedText;
    private final int indexOfQuestion;

    public TextOfAnswerOnQuestionTest(String expectedText, int indexOfQuestion) {
        this.expectedText = expectedText;
        this.indexOfQuestion = indexOfQuestion;
    }

    @Parameterized.Parameters
    public static Object[][] getQuestions() {
        return new Object[][] {
                {"Сутки — 400 рублей. Оплата курьеру — наличными или картой.", 0},
                {"Пока что у нас так: один заказ — один самокат. Если хотите покататься с друзьями, можете просто " +
                        "сделать несколько заказов — один за другим.", 1},
                {"Допустим, вы оформляете заказ на 8 мая. Мы привозим самокат 8 мая в течение дня. Отсчёт времени аренды" +
                        " начинается с момента, когда вы оплатите заказ курьеру. Если мы привезли самокат 8 мая в 20:30," +
                        " суточная аренда закончится 9 мая в 20:30.", 2},
                {"Только начиная с завтрашнего дня. Но скоро станем расторопнее.", 3},
                {"Пока что нет! Но если что-то срочное — всегда можно позвонить в поддержку по красивому номеру" +
                        " 1010.", 4},
                {"Самокат приезжает к вам с полной зарядкой. Этого хватает на восемь суток — даже если будете кататься " +
                        "без передышек и во сне. Зарядка не понадобится.", 5},
                {"Да, пока самокат не привезли. Штрафа не будет, объяснительной записки тоже не попросим. Все же свои.", 6},
                {"Да, обязательно. Всем самокатов! И Москве, и Московской области.", 7},
        };
    }

    @Before
    public void createDriver() {
        WebDriverManager.chromedriver().setup();
        driver = new ChromeDriver();

        //WebDriverManager.firefoxdriver().setup();
        //driver = new FirefoxDriver();

        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(EnvConf.IMPLICIT_WAIT));
    }

    @Test
    public void checkTextOfAnswerOnQuestionTest() {
        openHomePage();
        HomePageLocators locators = new HomePageLocators();
        scrollToElement(locators.getQuestion(indexOfQuestion));
        clickOnQuestion(locators);
        checkTextOfAnswer(locators);
    }

    @After
    public void tearDown() {
        driver.quit();
    }

    private void checkTextOfAnswer(HomePageLocators locators) {
        new WebDriverWait(driver, Duration.ofSeconds(EnvConf.EXPLICIT_WAIT)).until(ExpectedConditions
                .visibilityOfElementLocated(locators.getAnswer(indexOfQuestion)));
        assertEquals("Неверный текст", expectedText, driver.findElement(locators.
                getAnswer(indexOfQuestion)).getText());
    }

    private void clickOnQuestion(HomePageLocators locators) {
        driver.findElement(locators.getQuestion(indexOfQuestion)).click();
    }

    private void scrollToElement(By path) {
        WebElement element =  driver.findElement(path);
        ((JavascriptExecutor)driver).executeScript("arguments[0].scrollIntoView();", element);
    }

    public void openHomePage() {
        driver.get(EnvConf.BASE_URL);
    }

}
